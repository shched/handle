<?php
	$z = array(   // занести в массив значение полей
		1 => $_POST['name'],
		2 => $_POST['email'],
		3 => $_POST['content']
	);
	$dl = '';
	if($z[1] && $z[2] && $z[3]){
		mail("e3-reg@ya.ru", "заполнена форма site.ru", $z[1] . "\n" . $z[2] . "\n" . $z[3]);   // сообщение на эл. почту о новом комментарии
	if(strpos($z[3], 'http://') === false){   // если в тексте отзыва нет http://
		...
		Header("Location: ".$_SERVER['PHP_SELF']);   // обновить страницу; обновленная версия содержит опубликованный комментарий
		exit;
	}
